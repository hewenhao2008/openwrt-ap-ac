...
if(sendto(sock, cmdbuff, cmd_len,0,(struct sockaddr*)&toAddr,sizeof(toAddr)) != cmd_len)
{
 ...
 exit(1);
	...
	printf("web get ap info\n");
while(1)
{
	fromLen = sizeof(fromAddr);
	cmd_len = recvfrom(sock,recvBuffer,128,0,(struct sockaddr*)&fromAddr,&fromLen);
	if(cmd_len <0)
	{
	...
	hexlog((unsigned char *)recvBuffer, cmd_len);

	acbuf = (dcma_udp_skb_info_t *)recvBuffer;
	if( acbuf->type == 0)
	{
		break;
	}
