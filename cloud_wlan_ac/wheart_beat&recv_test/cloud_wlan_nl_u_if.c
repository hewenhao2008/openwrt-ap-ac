...
s32 cloud_wlan_nl_set_data(s8 *buff, u32 datalen)
{
	...
	datalen = datalen>MAX_PAYLOAD?MAX_PAYLOAD:datalen;
...
{
	/* 接收消息并打印 */  
	memset(g_cw_nl_info.nlh, 0, NLMSG_SPACE(MAX_PAYLOAD));  
...
s32 cloud_wlan_nl_recv_kmod(dcma_udp_skb_info_t *buff)
{
	...
	memcpy(buff->data, NLMSG_DATA(g_cw_nl_info.nlh), MAX_PAYLOAD); 
